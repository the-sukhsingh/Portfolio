...
const About = () => {
  ...
  useGSAP(()=>{
    ...
    gsap.fromTo("#about", {opacity:0}, {opacity:1, duration:2, scrollTrigger:{trigger:"#about", start:"top 80%", end:"top 30%", scrub:1}})
    ...
    <div ref={about}>
      ...
      <div className="w-[90%] lg:w-4/5 lg:min-h-[80vh] flex justify-around gap-5 items-center m-auto bg-gray-900 mb-12 p-5 rounded-lg flex-col lg:flex-row ">
        <div className="h-[50vh] lg:w-[50%] lg:h-[82%] text-[20px] bg-gray-800 p-8 rounded-md overflow-y-scroll lg:overflow-hidden">
          ...
          I have learned the importance of clean code, responsive design, and user
          experience. Beyond coding, I am always eager to learn new technologies
        ...
        <div className="w-full lg:w-1/2 min-h-[82%] grid grid-cols-4 justify-center items-center gap-[1rem] text-[22px] bg-gray-800 p-8 rounded-md">
          {Object.keys(skill).map((keyy,value) => {
            if (skill[keyy]) {
              return (<>
                <div className="hover-div" key={value}>
                  <img
                    className="skillLogo"
                    src={`logos/${keyy.toLowerCase()}.svg`}
                    alt=""
                    />
                  <div className="hover-box">{keyy.toUpperCase()}</div>
                </div>
                    </>
              );
            }
